...
\begin{flushleft}
{\Large
\textbf\newline{Accuracy of real-time multi-model ensemble forecasts for seasonal influenza in the U.S.}
% Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
% Insert author names, affiliations and corresponding author email (do not include titles, positions, or degrees).
\\
Nicholas G Reich\textsuperscript{1*}, % 
Craig J McGowan\textsuperscript{2}, %
Teresa K Yamana\textsuperscript{3}, %
Abhinav Tushar\textsuperscript{4}, 
Evan L Ray\textsuperscript{5}, %
Dave Osthus\textsuperscript{6}, %
Sasikiran Kandula\textsuperscript{3}, %
Logan C Brooks\textsuperscript{7}, %
Willow Crawford-Crudell\textsuperscript{8}, 
Graham Casey Gibson\textsuperscript{1}, %
Evan Moore\textsuperscript{1}, %
Rebecca Silva\textsuperscript{9}, %
Matthew Biggerstaff\textsuperscript{2}, %
Michael A Johansson\textsuperscript{10}, %
Roni Rosenfeld\textsuperscript{11}, %
Jeffrey Shaman\textsuperscript{3} %
\\
\bigskip
\textbf{1} Department of Biostatistics and Epidemiology, University of Massachusetts-Amherst, Amherst, MA, USA
\\
\textbf{2} Influenza Division, Centers for Disease Control and Prevention, Atlanta, GA, USA
\\
\textbf{3} Department of Environmental Health Sciences, Columbia University, New York, NY, USA
\\
\textbf{4} School of Computer Science, University of Massachusetts-Amherst, Amherst, MA, USA
\\
\textbf{5} Department of Mathematics and Statistics, Mount Holyoke College, South Hadley, MA, USA
\\
\textbf{6} Statistical Sciences Group, Los Alamos National Laboratory, Los Alamos, NM, USA
\\
\textbf{7} Computer Science Department, Carnegie Mellon University, Pittsburgh, PA, USA
\\
\textbf{8} Department of Mathematics and Statistics, Smith College, Northampton, MA, USA
\\
\textbf{9} Department of Mathematics and Statistics, Amherst College, Amherst, MA, USA
\\
\textbf{10} Division of Vector-Borne Diseases, Centers for Disease Control and Prevention, San Juan, PR, USA
\\
\textbf{11} Machine Learning Department, Carnegie Mellon University, Pittsburgh, PA, USA
\\
\bigskip
...
test_scores_by_model_target_week <- scores_adj %>%
    filter(Season == "2017/2018") %>%
    group_by(Model, Target, Epiweek, `Model Week`, Year) %>%
    summarize(
        avg_logscore = mean(score_adj),
        avg_score = exp(avg_logscore)
    ) %>%
    ungroup() %>%
  mutate(epidate = MMWRweek2Date(Year,Epiweek))
@
...
The FluSight Network assembled 21 component forecasting systems to build multi-model ensembles for seasonal influenza outbreaks (Table A in S1 Text).
...
Twenty-one individual component models were fit to historical data and used to make prospective forecasts during seven training seasons (2010/2011 - 2016/2017, Table A in S1 Text). 
...
We pre-specified five candidate ensemble approaches prior to any systematic evaluation of ensemble component performance in previous seasons and prior to the 2017/2018 season (Table A in S1 Text) \cite{Reich2017github}. 
...
The {\tt FSNetwork-TTW} model consistently outperformed a simpler ensemble model and the seasonal average model across all weeks of the 2017/2018 season (Fig F in S1 Text).
...
pal <- RColorBrewer::brewer.pal(7, "Blues")[-7]
...
p2 <- ggplot(tmp, aes(x=Model, y=Season, fill=factor(season_rank))) +
    ...
    scale_fill_manual(name="rank", values = rev(pal)) + 
    xlab(NULL) + ylab(NULL) +
    labs(x = NULL, y = NULL) + #, tag="B") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
Overall, the {\tt FSNetwork-TTW} model ranked second among selected models in both RMSE and average bias, behind the {\tt LANL-DBM} model (Fig A in S1 Text).
...
\Sexpr{specify_decimal(filter(ests_summaries, model_name=="target-type-based-weights", target=="Season onset")$bias,2)}
...
\Sexpr{specify_decimal(filter(ests_summaries, model_name=="target-type-based-weights", target=="1 wk ahead")$bias,2)}
ILI\%).


According to the probability integral transform metric\cite{angus1994probability,diebold1997evaluating}, the {\tt FSNetwork-TTW} model was well-calibrated for all four week-ahead targets (Fig B in S1 Text).
...
Over the entire training period prior to the 2017/2018 season, the {\tt FSNetwork-TTW} model calibration results suggested that in general the model was a bit conservative, with often a too wide predictive distribution (Fig C in S1 Text).
...
Components that received substantial weight in the original ensemble but did particularly poorly in the 2017/2018 season saw the largest drop in weight (Fig D in S1 Text). 
...
During the training and test phases, the weighted ensemble approaches outperformed two equal weight ensembles: one constructed based on FluSight Network models presented here (Table A in S1 Text) and one constructed by the CDC using a wider array of models \cite{McGowan2018}. 
...
We also assessed point estimate accuracy as a secondary measure (Fig A in S1 Text). 
...
Because we had sufficient cross-validation data to estimate ensemble weights for several dozen components, some groups contributed non-smoothed ``complementing'' components for that purpose (Table A in S1 Text).  
...
Examples of these approaches include the component models from Columbia University that use, e.g. Ensemble Average Kalman Filtering, to take weighted averages of model realizations to form predictive distributions (Table A in S1 Text).
Second, multi-model ensembles combine component models through techniques such as model stacking (see Methods).
Among the models described in this work, one component model ({\tt Delphi-Stat}) is a multi-model ensemble and all of the FluSight Network models are also multi-model ensembles (Table A in S1 Text).
...
A total of five ensemble weighting schemes were considered, with varying complexity and number of estimated weights (Table A in S1 Text).
\begin{itemize}
\item 
...
Code for specific models are either publicly available or available upon request from the modeling teams, with more model-specific details available at the related citations (Table A in S1 Text).
...
<<ensemble-test-train-comparison, fig.show="hide", fig.height=3>>=
require(stringr)
pal <- RColorBrewer::brewer.pal(6, "Blues")[-c(6)]
...
p2 <- ggplot(tmp, aes(x=Model, y=Season, fill=factor(season_rank))) +
    geom_tile() +
    geom_text(aes(label = specify_decimal(exp(avg_logscore),3)), fontface="bold") +
    #scale_fill_brewer(name="rank", direction=-1) + 
    scale_fill_manual(name="rank", values = rev(pal)) + 
    xlab(NULL) + ylab(NULL) +
    labs(x = NULL, y = NULL) + #, tag="B") +
...
<<scores-by-week, fig.show="hide", fig.width=9>>=

## make figure showing scores over time in test-season

models_to_plot <- c("FSNetwork-EW", "FSNetwork-TTW", "ReichLab-KDE")

dat_to_plot <- filter(test_scores_by_model_target_week, 
  Model %in% models_to_plot)

ggplot(dat_to_plot, aes(x=epidate, y=avg_score, color=Model)) +
  facet_wrap(.~Target, ncol = 4) +
  geom_line() +
  scale_color_brewer(palette="Dark2") +
  ylab("average forecast score") + xlab("2017/2018 epidemic week") +
  scale_x_date(date_breaks = "6 weeks", date_labels = "EW%W") +
  geom_vline(xintercept=as.Date("2018-01-01"), linetype=2, color="darkgrey") +
