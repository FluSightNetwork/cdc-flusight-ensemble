...
selected_models <- c("FSNetwork-TTW", "Delphi-Stat", "ReichLab-KCDE", "LANL-DBM", "CU-EKF_SIRS")
...
\begin{itemize}
 \item compare all ensembles with single best model from each team in CV stage and with unweighted average from FSN from 2015/2016 - 2017/2018
 \item also, consider comparison with ensemble from limited set from each team, i.e. the single standalone or best model.
\end{itemize}
