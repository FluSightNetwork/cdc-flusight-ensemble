...
We assembled 21 ensemble components to be evaluated for inclusion in our ensemble models (Table \ref{tab:model-list}).
...
Individual component forecast performance in the seven training seasons (2010/2011 - 2016/2017) varied widely across region, season, and target.
A detailed comparative analysis of component forecast performance can be found elsewhere (Reich et al, under review), however we summarize a few key insights from model performance here.
...
Over 50\% of the ensemble components out-performed the seasonal baseline model in forecasting 1- and 2-week ahead incidence as well as season peak percentage and season peak week.

%\subsubsection*{Quantitative summaries of how much forecasting data collected and how stored/scored}

Each ensemble component's weekly forecasts were submitted in real-time to a central data repository throughout the 2017/2018 influenza season.\cite{FSNGithub2018}
...
We chose the target-type weights model as the model that would be submitted in real-time to the CDC during the 2017/2018 season, based on it having the highest score of any approach in the cross-validated training phase.
...
ttweights_2017 <- filter(target_type_weights, season=="2018/2019", target %in% c("1 wk ahead", "Season onset")) %>% #filter(target_type_weights, season=="2017/2018") %>%
...
In spite of these unusual dynamics negatively impacting the forecast accuracy of the top-performing ensemble components, the {\tt FSNetwork-TTW} ensemble model that varied weights by model and target-type was tied for the best performance among all component and ensemble models in 2017/2018.
...
new_ttws <- data.frame(
    ...
    target_type = rep(c("week-ahead", "seasonal"), times=c(length(dem_weights_wkahead), length(dem_weights_seasonal)))
    ) %>% 
    mutate(component_model_id = reorder(component_model_id, updated_weight))

# ggplot(new_ttws, 
#     aes(y=target_type, fill=updated_weight, x=component_model_id)) + 
#     geom_tile() + ylab(NULL) + xlab(NULL) +
...
\begin{itemize}
  ...
  \item Overall, the ensemble methods outperform ensemble components. There are some specific situations where the ensemble components do better, but that is to be expected when testing a lot of situations.
  \item Even in a very unusual season, the ensemble approach holds steady and was high-performing. This bodes well for its long-term robustness, compared to single ensemble components. In this way, the relative simplicity of CW or TTW is a strength, as there is not too much danger of these models being "overfit" to the data.
