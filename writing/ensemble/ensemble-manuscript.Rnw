...
In particular, we selected the single best model from each team in training phase stage and the {\tt FluSight-unweighted_average} model to compare with the {\tt FSNetwork-TTW} model (Figure \ref{fig:season-average-results}).
...
Despite being optimized for high log-score values, the {\tt FSNetwork-TTW} showed robust performance in the 2017/2018 season across other performance metrics that measure forecast calibration.
% Its root mean squared error was XX XX [add rank raster?] (See Appendix).
% ADD BIAS!!
According to the probability integral transform metric\cite{angus1994probability,diebold1997evaluating}, the {\tt FSNetwork-TTW} model was well-calibrated for all four week-ahead targets showing no significant deviations (see Appendix).
...
[[Add results for overall PIT performance across all seasons?]]
...
<<rmse-and-bias, fig.height=4, fig.show="hide">>=
...
p2 <- ggplot(ests_summaries, aes(x=model_id, y=target, fill=factor(bias_rank))) +
    ...
    scale_fill_brewer(name="rank") + 
    xlab(NULL) + ylab(NULL) +
    labs(x = NULL, y = NULL, tag="B: average bias") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
\section{Ensemble performance over time}

[[Plot showing performance over time relative to peak in 2018 (x), one line for each selected model, avg logscore (y)]]
