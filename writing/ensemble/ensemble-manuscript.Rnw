...
A season typically consists of forecast files generated weekly for 33, starting with epidemic week 40 (EW40) of one calendar year and ending with EW20 of the following year.
A model's weekly submission contained probabilistic and point forecasts for seven targets in each of 11 regions in the U.S. (national-level plus the 10 Health and Human Services (HHS) regions, Figure \ref{fig:overview-schematic}A).
Targets can be thought of as being of two types: ``week-ahead'' and ``seasonal''.
``Week ahead'' targets refer to the four weekly incidence targets that are different for each week of the season.
``Seasonal'' targets refer to quantities that do not change for a region within a season (see Figure \ref{fig:overview-schematic}B and Methods). 
...
This measure, which we refer to as ``forecast score'', can be interpreted as the average probability a given forecast model assigned to the true value.
As such, higher values, on a scale of 0 to 1, indicate more accurate models. 
In the seven training seasons, we observed that around 2/3 of the component models reliably out-performed a seasonal average model on all targets. 
...
For each of the 33 weeks of the 2017/2018 season, each of 21 models submitted weekly probabilistic forecasts for 7 targets in each of 11 regions. 
In all, the seven seasons of training data consisted of about 95GMB and over 1.5m rows of data per model and about 2GB of forecast data for all models combined.
...
The simplest ensemble took a simple equal-weight average of all models regardless of past performance, essentiall only assigning all models the same weight.
...
The {\tt \Sexpr{top_training_models[1]}} model outperformed all other ensemble and component models in the training phase by a slim margin, achieving an average forecast score of 
...
For seasonal targets, the highest non-zero weight 
...
In the weights for the seasonal targets, the 6 models shared fairly equal weights, with none of the 6 having less than 0.11 weight.
...
In spite of these unusual dynamics impacting strongly the performance of some of the top-performing component models, the {\tt FSNetwork-TTW} ensemble model that varied weights by model and target-type was tied for the best performance among all component and ensemble models in 2017/2018.
...
These models outperformed all other ensemble and component models in the training phase by a slim margin.
...
\subsection*{Post-hoc analysis of dynamic weighting schemes (discussion of NN, "follow-the-leader", ...)}

[Not sure if we should include analyses from Abhinav: Post-hoc analyses of alternative ensemble weighting schemes that followed a more in-season adaptive  approach to assigning weights to specific models showed that such approaches would have provided some marginal benefit over the chosen approaches.]
...
\begin{itemize}
  ...
  \item Even in a very unusual season, the ensemble approach holds steady and was high-performing. This bodes well for it's long-term robustness, compared to other single component models. In this way, the relative simplicity of CW or TTW is a strength, as there is not too much danger of these models being "overfit" to the data.
