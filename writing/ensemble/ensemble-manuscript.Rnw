...
\author{Nicholas G Reich$^1$, Logan Brooks$^2$, Sasikiran Kandula$^3$, Craig McGowan$^4$,\\Evan Moore$^1$, Dave Osthus$^5$, Evan Ray$^6$, Abhinav Tushar$^1$, Teresa Yamana$^3$,\\
...
The FluSightNetwork's target-type-weights ({\tt FSNetwork-TTW}) model estimated 42 weights, one for each model and target-type (week-ahead and seasonal) combination.
...
the {\tt \Sexpr{top_training_models[4]}} model with a score of \Sexpr{specify_decimal(exp(top_training_scores[4]),2)}.
...
Using out-of-sample cross-validated performance of all component models across the seven training seasons, we estimated weights for the four ensemble models with varying weights.
...
The {\tt \Sexpr{top_models[1]}} model outperformed all other ensemble and component models in the training phase by a slim margin, achieving an average forecast score of 
\Sexpr{specify_decimal(exp(top_scores[1]),2)}, 
compared with 
the {\tt \Sexpr{top_models[2]}} model with a score of \Sexpr{specify_decimal(exp(top_scores[2]),2)}, 
the {\tt \Sexpr{top_models[3]}} model with a score of \Sexpr{specify_decimal(exp(top_scores[3]),2)}, and 
the {\tt \Sexpr{top_models[4]}} model with a score of \Sexpr{specify_decimal(exp(top_scores[4]),2)}.

[Describe additional performance metrics: bias and PIT results]
