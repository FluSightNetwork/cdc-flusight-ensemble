...
Table 1: model table with ensembles
Figure 1: Flu Forecasting/Ensemble Overview
Figure 2: Seasonal average Results figure for all models, all seasons
Figure 3: Model weights
Figure 4: Results for 2017/2018: by region or target?

Suppl Figure: CV and real-time results for ensemble models, sorted by complexity
...
The {\tt FSNetwork-TTW} model assigned non-negligible weight to XX models for week-ahead targets and XX models for seasonal targets (Figure \ref{fig:ttw-weights}).
