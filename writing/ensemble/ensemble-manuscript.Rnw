...
[Background paragraph on flu, forecasting, and public health outbreak response]
...
The FluSightNetwork's target-type-weights ({\tt FSNetwork-TTW}) model estimated 42 weights, one for each model and target-type (week-ahead and seasonal) combination (Figure \ref{fig:ttweights}).
...
compared with 
the {\tt \Sexpr{top_training_models[2]}} model with a score of \Sexpr{specify_decimal(exp(top_training_scores[2]),2)}, 
the {\tt \Sexpr{top_training_models[3]}} model with a score of \Sexpr{specify_decimal(exp(top_training_scores[3]),2)}, and 
the {\tt \Sexpr{top_training_models[4]}} model with a score of \Sexpr{specify_decimal(exp(top_training_scores[4]),2)} (Figure \ref{fig:season-average-results}).
...
[detailed explanation of the 5 models]

[cross-validation/real-time data]
