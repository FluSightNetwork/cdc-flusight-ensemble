...
\author{(working author list subject to re-ordering pending final contributions)\\Nicholas G Reich$^1$, Logan Brooks$^2$, Sasikiran Kandula$^3$, Craig McGowan$^4$,\\Evan Moore$^1$, Dave Osthus$^5$, Evan Ray$^6$, Abhinav Tushar$^1$, Teresa Yamana$^3$,\\
Willow Crawford-Crudell$^7$, Graham Casey Gibson$^1$, Rebecca Silva$^8$\\Matthew Biggerstaff$^4$, Michael A Johansson$^9$, Roni Rosenfeld$^2$, Jeffrey Shaman$^3$}
...
This research consortium worked collaboratively throughout 2017 and 2018 to build and implement in real-time a multi-institution ensemble, with the goal of outperforming the ``simple average'' ensemble that CDC uses to inform decision making and situational awareness during the annual influenza season. 
Given the success in previous years of a simple ensemble that incorporated no prior information about the relative performace of component models, the FluSight Network's main goal was to show that an ensemble that uses validated weights based on models' past performance could outperform this simple averaging approach. 
This research is an important example of a collaboration between government and academic public health experts to systematically test if information for decision making during a high severity influenza season can be improved..  
...
\subsection{Summary of component models} \label{subsec:comp-models}

%\subsubsection*{General description of models included, targets, regions, etc. }
We assembled 21 component models to be evaluated for inclusion in our ensemble models (Table \ref{tab:model-list}).
...
Weekly submissions contain probabilistic and point forecasts for seven targets in each of 11 regions in the U.S. (national-level plus the 10 Health and Human Services (HHS) regions, Figure \ref{fig:overview-schematic}A).
...
A detailed comparative analysis of component model forecast performance can be found elsewhere (Reich et al, under review), however we summarize a few key insights from model performance here.
...
Prior to any systematic evaluation of component model performance and prior to the 2017/2018 season, we pre-specified five ensemble models and evaluated their performance in the seven previous training seaons.
The five pre-specified ensemble models all were built using weighted model averages of the component models, using a predictive density stacking approach (see Methods).
...
Across the seven training seasons, all ensemble models but the simple average showed higher overall average forecast score (i.e., averaged across all targets) than any single component model (Figure \ref{fig:season-average-results}). 

<<season-average-results, fig.height=5, fig.cap="Average forecast score, aggregated across targets, regions, and weeks, plotted separately for each model and season. Models are sorted from lowest scores (left) to highest scores (right). Higher scores indicate better performance. Dots show average scores across all targets, regions, and weeks within a given season. The `x' marks the geometric mean of the seven seasons. The black dot represents the average score for the prospective, real-time forecasts from the 2017/2018 season. The {\\tt ReichLab-KDE} model (bold, italicized font) is considered the historical baseline model. The models highlighted in red, boldface font are the FluSight Network ensemble models. Note that a component’s standalone accuracy does not necessarily correlate to its contribution to the overall ensemble accuracy.  See discussion at end of Section \ref{subsec:comp-models}.">>=
...
ggplot(tmp, aes(x=Model, y=exp(avg_logscore))) +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5,
        ...
        face=ifelse(
            ...
            "bold.italic",
            ifelse(grepl("FSNetwork", levels(tmp$Model)),
            "bold",
            "plain")
            ),
        color = ifelse(
            grepl("FSNetwork", levels(tmp$Model)),
            "red",
            "black"
        )
        ))
...
The {\tt \Sexpr{top_training_models[1]}} model outperformed all other ensemble and component models in the training phase by a slim margin. 
This model achieved an average forecast score of 
...
The \Sexpr{model_name_abv$Name[model_name_abv$Model == "FSNetwork-TTW"]} ({\tt FSNetwork-TTW}) model estimated 42 weights, one for each model and target-type (week-ahead and seasonal) combination (Figure \ref{fig:ttweights}).
...
Using out-of-sample cross-validated performance of all component models across the seven training seasons, we estimated weights for the chosen {\tt FSNetwork-TTW} ensemble model that would be used for the 2017/2018 real-time forecasting.
...
In the weights for the seasonal targets, 6 models shared fairly equal weights, with none of the 6 having less than 0.11 weight.
All four teams had at least one model with non-negligible weight in the chosen model. 
...
As measured by wILI percent at the national level, the 2017/2018 season was on par with the other two highest peaks on record: the 2003/2004 season and the 2009 H1N1 pandemic.
...
In spite of these unusual dynamics negatively impacting the forecast accuracy of the top-performing component models, the {\tt FSNetwork-TTW} ensemble model that varied weights by model and target-type was tied for the best performance among all component and ensemble models in 2017/2018.
...
These results from 2017/2018 were consistent with and confirmed conclusions drawn from the training period, where these same models outperformed all other ensemble and component models.
This strong and consistent performance by the ensemble models is a particularly noteworthy achievement given that just matching the single model that happens to do best in a particular season (without being a pre-specified comparison) is difficult. 
...
ggplot(
    ...
    geom_text(aes(label=specify_decimal(avg_score, 2)), size=1.8) +
    scale_fill_gradient2(name="% change \nfrom baseline") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5))
@


[Describe additional performance metrics: bias and PIT results from Rebecca.]
...
[Could also present/describe updated weights that incorporate 2017/2018 performance.]
...
Outline
\begin{itemize}
  \item something about CDC using these data to decide whether or not to recommend the new ensemble approach for the 2018-19 season and how the weights will be updated going forward. 
  \item something about how this is a collaboration between CDC and outside partners and how we were able to accomplish more by collaboration in real time on a common problem.
  \item ensembles hold promise for giving decision makers the ability to use "one answer" that combines the strength of many different approaches. highlight that no one model was ever used by the ensemble as the best answer and instead it relied on a combination of models to give the best answer.   \item i do a think limitation or a something to consider working on is how to adapt these methods when there aren't historic analogous (e.g. ebola, pandemic flu)
  ...
  \item Even in a very unusual season, the ensemble approach holds steady and was high-performing. This bodes well for its long-term robustness, compared to single component models. In this way, the relative simplicity of CW or TTW is a strength, as there is not too much danger of these models being "overfit" to the data.
  \item Important limitation/opportunity: not an obvious way to incorporate new models that, say, rely on a new data source and don't have 7 years of training. Could be important to include such a model, but how to do it is not trivial, and won't have a good sense about how the long run performance of this model is.
  ...
  \item We did not explore the use of more complex regularization schemes for estimation in this work, but in future work penalized estimation procedures could be employed for the more complex models, yielding an intermediate number of effective parameters.  Opportunity to cite awes.
  \item This new approach shows terrific promise for future collaborative forecasting efforts. Was a close second place in the 2017/2018 competition. Highlights the value of standard data formats and requiring multiple "training seasons".
\end{itemize}
...
where $\pi_{c, t, r, w}$ is the weight assigned to component $c$ for predictions of target $t$ in region $r$ on week $w$. We require $\sum^C_{c = 1} \pi_{c,t,r} = 1$ and thereby ensure that $f(y_{t,r,w})$ remains a valid probability distribution. 
