...
<<rmse, fig.height=4, fig.cap="RMSE by model and target, for targets on ILI \\% scale.">>=
...
ests_summaries <- tbl_df(point_ests) %>% 
    filter(model_name != "UTAustin_edm") %>%
    dplyr::left_join(
        all_target_bounds, 
    ...
    mutate(
        model_id = reorder(`model-id`, X=-rmse),
        target_type = ifelse(grepl("Season", target), "seasonal", "weekahead")
        ) 
...
ggplot(ests_summaries, aes(x=model_id, y=rmse, color=target)) + 
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5,
        face=ifelse(
            levels(ests_summaries$model_id) == "ReichLab-KDE",
            "bold.italic",
            ifelse(grepl("FSNetwork", levels(ests_summaries$model_id)),
                "bold",
                "plain")
        ),
        color = ifelse(
            grepl("FSNetwork", levels(ests_summaries$model_id)),
            "red",
            "black"
        )
    )) +
...
[MSE is lower for ensembles Figure \ref{fig:rmse}, etc...]
...
[MSE evaluation of point predictions. median of probabilistic distribution used for ensemble models.]
