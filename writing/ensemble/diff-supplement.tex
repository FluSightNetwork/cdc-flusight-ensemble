...
%DIF DEL ensemble-supplement-pcb-submitted.tex   Fri Aug 30 10:36:00 2019
%DIF ADD ensemble-supplement.tex                 Wed Sep 11 20:37:00 2019
%DIF 2-3c2-3
%DIF < %% maxwidth is the original width if it is less than linewidth
%DIF < %% otherwise use linewidth (to make sure the graphics do not exceed the margin)
%DIF -------
...
\DIFaddbegin \section{\DIFadd{Comparison of ensemble model performance in testing and training phases}}

\DIFadd{To compare the training and test phase performance of the ensemble models, we plotted their relative performance (Figure \ref{fig:ensemble-test-train}). This shows that the }{\tt \DIFadd{FSNetwork-TTW}} \DIFadd{model had the highest overall score in both the training and test phase.
}

\begin{figure}[htbp]
\begin{center}
\includegraphics{figure/ensemble-test-train-comparison-1.pdf}
\caption{\DIFaddFL{Overall test and training phase performance scores for the five ensemble models. Displayed scores are averaged across targets, regions, and weeks, and plotted separately for selected models. Model ranks within each row are indicated by color of each cell (darker colors indicate higher rank and more accurate forecasts) and the forecast score (rounded to three decimal places) is printed in each cell.}}
\label{fig:ensemble-test-train}
\end{center}
\end{figure}


\section{\DIFadd{Forecast accuracy across the 2017/2018 test season}}

\DIFadd{We compared the average weekly accuracy of three models --}{\tt \DIFadd{FSNetwork-TTW}}\DIFadd{, }{\tt \DIFadd{FSNetwork-EW}}\DIFadd{, and }{\tt \DIFadd{ReichLab-KDE}}\DIFadd{-- over time in the 2017/2018 test season (Figure \ref{fig:weekly-accuracy}). This comparison showed that a performance-weighted ensemble, }{\tt \DIFadd{FSNetwork-TTW}}\DIFadd{, consistently outperformed an equally weighted ensemble, }{\tt \DIFadd{FSNetwork-EW}}\DIFadd{, by a small margin and a seasonal average model, }{\tt \DIFadd{ReichLab-KDE}}\DIFadd{, by a larger margin. Accuracy for week-ahead targets was substantially lower in the middle of the season.
}

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=\linewidth]{figure/scores-by-week-1.pdf} 
\caption{\DIFaddFL{Forecast scores averaged across all regions for three selected models in the 2017/2018 test season. The x-axis shows the epidemic week (EW) of the season. Scores are only shown for scored weeks according to FluSight guidelines (see Methods in original manuscript). Therefore, season onset scores are truncated six weeks after the last season onset occurred. A vertical dashed line indicates January 1, 2018.}}
...
\DIFaddend \section{EM Algorithm for Weighted Density Ensembles}
...
\begin{eqnarray}
{\mathbf E}[ \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{\mbox }\DIFdelend model\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend _c |data ] % & = & \sum_i Pr(Model_c | z_i^*) \\ 
 %& = & \sum_i \frac{Pr(Model_c, z_i^*)}{Pr(z_i^*)} \\ 
 %& = & \sum_i \frac{Pr(Model_c) Pr(z_i^*|model_c)}{f(z_i^*)} \\ 
 & = & \sum_i \frac{\pi_c f_c(z_i^*)}{f(z_i^*)} 
\end{eqnarray}
Heuristically, we can think of the expression $f_c(z)$ equivalently as \DIFdelbegin \DIFdel{$Pr(z| model c)$ }\DIFdelend \DIFaddbegin \DIFadd{$Pr(z| model_c)$ }\DIFaddend or in words the likelihood of seeing the value $z$ given that component $c$ is the ``chosen'' model. 
...
\begin{eqnarray}
\pi_c^{(k+1)} & = & \frac{1}{N} {\mathbf E}[ \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{\mbox }\DIFdelend model\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend _c |data ] \\
