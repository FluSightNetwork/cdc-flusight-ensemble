...
In addition to examining where our models perform well, the forecast data assembled by this effort allows us to identify places where current state-of-the-art forecast models still need improvement. We identify and quantify several of these challenges, including revisions to initially reported data, ....
...
In all, XX\% of all weekly reported wILI percentages ended up being over 20\% different than the originally reported value.
We found substatial evidence supporting a strong inverse association between the amount of bias in the initial reported data and the forecast skill for the forecasts made using the biased data.
Specifically, adjusting for model, week-of-year, and target, among top-performing models we see a XX decline in forecast skill when the first observed wILI measurement is XX (Figure \ref{fig:delay-model-coefs}).
