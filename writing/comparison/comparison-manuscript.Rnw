...
Since each season has unique dynamical structure, multi-season comparisons like this will greatly improve our understanding of how models perform over the longer term.
...
Building on the structure of the FluSight challenges (and those of other collaborative forecasting efforts\cite{Viboud}), a subset of FluSight participants founded a consortium to facilitate direct comparison and fusion of modeling approaches. 
...
\subsubsection*{Analysis of delays}
...
Overall, forecast skill was lower for seasonal targets than for week-ahead targets (Figure \ref{fig:regults-model-region-tt}).
...
colnames(skills_to_show) <- c("target", "stat. model skill", "compartment model skill", "difference")
