...
scores_for_peak_accuracy <- scores %>%
    dplyr::filter(Model %in% complete_models, Target %in% c("Season peak week", "Season peak percentage")) %>%
    dplyr::left_join(peak_weeks, by = c("Season", "Location")) %>%
    dplyr::filter(`Model Week` >= peak_week-6, `Model Week` <= peak_week+6) %>%
    #dplyr::filter((`Model Week` >= pre_peak_start_week & `Model Week` <= pre_peak_end_week) | (`Model Week` >= post_peak_start_week & `Model Week` <= post_peak_end_week)) %>%
    ## if NA, NaN or <-10, set score to -10
    mutate(score_adj = dplyr::if_else(is.nan(!!SCORE_COL) | is.na(!!SCORE_COL) , 
        -10, 
        !!SCORE_COL)) %>%
    mutate(
        score_adj = dplyr::if_else(score_adj < -10 , -10, score_adj),
        Location = factor(Location, levels=c("US National", paste("HHS Region", 1:10))),
        Model = factor(Model, levels=levels(scores_by_model$Model), ordered=TRUE),
        Location = reorder(Location, score_adj),
    ...
    ungroup() %>%
    group_by(Model, Target, week_rel_to_peak) %>%
    summarize(
        avg_logscore = mean(score_adj),
        avg_logscore_cat = cut(exp(avg_logscore), breaks=seq(0,1, by=.1))
    ) %>%
    ungroup() %>%
...
# ggplot(scores_for_peak_accuracy, aes(x=week_rel_to_peak, fill=exp(avg_logscore), y=Model)) + 
#     geom_tile() + ylab(NULL) + xlab("week relative to peak") +
#     facet_grid(~Target) +
#     scale_x_continuous(breaks=-6:6) +
#     scale_fill_gradient(name="forecast score", limits=c(0,1), low = "#f7fbff", high="#08306b") +
#     geom_text(aes(label=specify_decimal(exp(avg_logscore), 2)), size=2) 

ggplot(scores_for_peak_accuracy, aes(x=week_rel_to_peak, fill=avg_logscore_cat, y=Model)) + 
    geom_tile() + ylab(NULL) + xlab("week relative to peak") +
    facet_grid(~Target) +
    scale_x_continuous(breaks=-6:6) +
    scale_fill_brewer(type="seq", name="average forecast score") +
...
#     left_join(comp_models, by=c("model_name" = "model-dir")) %>%
#     filter(
#         `model-id` %in% complete_models, 
#         target %in% c("Season peak week", "Season peak percentage"),
#         season != "2017/2018"
#         ) %>%
#     dplyr::left_join(peak_weeks, by = c("season"="Season", "location"="Location")) %>%
#     dplyr::filter(model.week >= peak_week-6, model.week <= peak_week+6) %>%
#     group_by(model_name, target, season) %>%
#     mutate(week_rel_to_peak = model.week - peak_week) %>%
#     ungroup() %>%
#     group_by(model_name, target, week_rel_to_peak) %>%
#     summarize(nobs = n(),
...
#     ungroup()
# 
# ggplot(ests_summaries, aes(y=model_name, x=week_rel_to_peak, fill=bias)) +
#     geom_tile() +
#     facet_grid(~target) +
#     scale_x_continuous(breaks=-6:6) +
...
colnames(skills_to_show) <- c("target", "stat. model", "comp. model", "diff.")
...
caption_text <- paste0("Comparison of the top three statistical models ({\\tt ",
    ...
    paste(sanitize(top_models[top_models%in%compartment_models]), collapse="}, {\\tt "),
    "}) based on best average region-season forecast score. The diff. column represents the difference in the average probability assigned to the eventual outcome for the target in each row. Positive values indicate the top statistical models showed higher average score than the top compartmental models.")
...
print(
    ...
    hline.after = c(0,nrow(skills_to_show)),
...
This rule was invoked for \Sexpr{sum(filter(scores_trimmed, Model %in% complete_models)$`Multi bin score`< -10)} scores, or \Sexpr{specify_decimal(sum(filter(scores_trimmed, Model %in% complete_models)$`Multi bin score`< -10)/nrow(scores_trimmed)*100, 1)}\% of all scores that fell within the scoring period.
