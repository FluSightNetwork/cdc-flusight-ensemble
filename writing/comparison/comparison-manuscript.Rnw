...
<<>>=
kde_skill <- unlist(scores_by_model[which(scores_by_model$Model=="ReichLab-KDE"), "avg_score"])
max_skill <- max(scores_by_model$avg_score)
min_skill <- min(scores_by_model$avg_score)
