...
[[NEED SNAPPY CLOSING SENTENCE HERE]]
...
\begin{itemize}
    \item On the whole, models improved substantially on baseline forecasts, both in regions with more consistent seasonal trends and those with less consistent seasonal trends (Figure \ref{fig:results-model-region-tt}B, D);
    \item At the presented spatial and temporal resolutions for influenza forecasts, we did not identify a meaningful difference between high-performing models that rely on an underlying mechanistic (i.e., compartmental) model and those that are more statistical in nature (Section \ref{sec:stat-mech});
