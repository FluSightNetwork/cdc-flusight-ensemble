...
\author{Nicholas G Reich$^1$, Logan Brooks$^2$, Spencer Fox$^3$, Sasikiran Kandula$^4$, \\Craig McGowan$^5$, Evan Moore$^1$, Dave Osthus$^6$, Evan Ray$^7$, Abhinav Tushar$^1$, Teresa Yamana$^4$, \\Matthew Biggerstaff$^5$, Michael Johansson$^8$, Roni Rosenfeld$^2$, Jeffrey Shaman$^4$}
...
Seasonal outbreaks of influenza are common in temperate regions of the world, with highest incidence typically occuring in colder and dryer parts of the year. 
...
We compared forecast accuracy of each model against a historical baseline seasonal average.
...
Continued methodological and technological innovation are essential to enhance timely reporting and overall accessibility of novel and traditional data sources for forecasting.
These advances will continue to improve the extent to whih influenza forecasts can be successfully integrated with real-time public health decision-making.
...
While such multi-model comparisons exist in the literature for single-season performance \cite{Biggerstaff2016,Biggerstaff2018}, here we compare the same models over seven influenza seasons. 
...
Influenza typically exhibits strongly seasonal patterns in the US (and globally throughout the Northern Hemisphere), often circulating widely during colder months (i.e., November through April). 
The social, biological, environmental, and demographic features that contribute to higher-than-usual incidence in a season are not fully understood, although contributing factors may include temperature and humidity\cite{lowen2007}, vaccine effectiveness\cite{Rolfes2016}, severity of the dominant influenza subtype\cite{thompson2003}, or timing of school vacations\cite{cauchemez2008}.
...
The reported scores are aggregated into an average on the log scale and then exponentiated so the reported scores can be interpreted as the average probability assigned to the eventually observed value of each target by a particular model. 
...
First, what is the baseline score that a model based on historical averages can achieve? Or, in other words, how stable are a given regions' patterns from season to season?
...
The frequency and magnitude of revisions varied substantially by region.
...
Using models from some of the most experienced forecasting teams in the country, we have obtained several key observations about forecasting seasonal influenza in the US. 
\begin{itemize}
    ...
    \item A majority of the presented models showed consistent improvement over the historical baseline for 1 and 2 week-ahead forecasts, although fewer models consistently outperformed the baseline model for 3 and 4 week-ahead forecasts (Figure \ref{fig:results-model-region-tt}E).
    ...
    \item A significant impediment to improved forecasting in some regions is the reporting biases in initially reported real-time data (Figure \ref{fig:delay-analysis}).
\end{itemize}
...
Therefore, a disease transmission model and the wILI data-generating model are fundamentally different, suggesting a limitation to purely mechanistic models in this context.
...
Additionally, while seven seasons of forecasts from 22 models is the largest study we know of that compares models from multiple teams, this remains a smaller-than-desired sample size about model performance. 
...
In this work, we have identified and quantified some of the key challenges (specifically, challenges with timely reporting of surveillance data). 
However, other barriers may be equally or more important to continued improvement of forecasts.
...
A web applet with interactive visualizations of the model evaluations is available at \url{https://ermoore.shinyapps.io/FSN\_Model\_Comparison/}. 
