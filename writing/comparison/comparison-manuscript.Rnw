...
\title{Collaborative Multi-year Evaluation of Seasonal Influenza Forecasting Models in the U.S.}
...
The {\tt UTAustin-edm} and {\tt Delphi-DeltaDensity2} models also used variants of nearest-neighbors regression, although overall skill for these models was not as consistent, indicating that implementation and or input variables can impact the performance of this approach.
...
Average forecast skill for the four week-ahead targets varied substantially across models and regions (Figure \ref{fig:results-model-region-tt}).
The model with the most highest average week-ahead forecasts across all regions and seasons was {\tt \Sexpr{sanitize(max_skill_model_wkahead)}}.
...
## Get top six models for k-week-ahead
# scores_by_model_targettype
...
Looking at results from the top six models, Region XX was the easiest to forecast and XX was the hardest, with models showing an average forecast skill of XX and XX across all seasons, respectively.
