...
<<results-model-season, fig.cap="Average forecast skill, aggregated across targets and plotted separately for each model and season. Models are sorted from least skill (left) to most skill (right). Dots show average skill across all targets and regions for a given season. The x marks the geometric mean of the seven seasons. The names of compartmental models are shown in bold face. The ReichLab-KDE model can be thought of as the historical baseline model.">>=
...
best_region <- scores_by_region$Location[which.max(scores_by_region$avg_logscore)]
best_region_skill <- exp(scores_by_region$avg_logscore[which.max(scores_by_region$avg_logscore)])
worst_region <- scores_by_region$Location[which.min(scores_by_region$avg_logscore)]
worst_region_skill <- exp(scores_by_region$avg_logscore[which.min(scores_by_region$avg_logscore)])
...
<<map-results, fig.cap="(A) Color shading indicates average forecast skill for models that performed better than the historical baseline model. (B) Plot showing the average historical model skill (x-axis) against the difference between the skill of all models that performed better than the historical baseline model and the historical model (y-axis). Positive y-values indicate an improvement over the historical baseline. Numbers indicate the location of the points for each region.">>=
data("fifty_states")
data(hhs_regions)
scores_for_map <- scores_by_model_region %>% 
    filter(Location != "US National", Model %in% models_wkahead_better_than_kde) %>%
    group_by(Location) %>%
...
#     ggtitle("A: week-ahead forecast skill")
# 
# ggplot(kde_scores, aes(x=exp(kde_logscore), y=exp(avg_logscore))) + 
#     geom_text(aes(label=region_number)) +
#     geom_abline(slope=1, intercept=0)

wkahead_comparison <- ggplot(kde_scores, aes(x=exp(kde_logscore), y=exp(avg_logscore)-exp(kde_logscore))) + 
    geom_text(aes(label=region_number), fontface="bold") +
    geom_hline(yintercept=0, linetype=2) + 
    scale_y_continuous(limits=c(-.15, .15), name = "skill improvement over historical model") +
    scale_x_continuous(name = "historical model skill") +
    ggtitle("B: week-ahead skill comparison with historical forecast") #+
