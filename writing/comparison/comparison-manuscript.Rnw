...
Implementation of interventions designed to disrupt disease transmission, such as vector control measures or mandatory infection prevention protocols at hospitals or health clinics, could be targeted based on forecasted incidence.
...
Building on the structure of the FluSight challenges (and those of other collaborative forecasting efforts\cite{Smith2017,Viboud2017}), a subset of FluSight participants founded a consortium to facilitate direct comparison and fusion of modeling approaches. 
...
Detailed methodology and results from previous FluSight challenges have been published\cite{Biggerstaff2016,Biggerstaff2018}, but we summarize the key features of the challenge here.
...
During each influenza season, these data are updated each week by the CDC (Figure \ref{fig:timezero-schematic}). When the most recent data are released, the prior weeks' reported wILI data may also be revised. 
...
Some data sources (e.g. wILI data prior to [[2013]]) were not archived in a way that made data retrievable in this ``real-time'' manner. In these situations, teams were still allowed to use these data sources.
...
We represent one specific scalar modified log score value as $\log f_{m,r,t,s,w}(z^*|\bf{x})$. 
...
In all cases, we compute the averages arithmetically on the log scale and only exponentiate before reporting and interpreting a final number; therefore, all reported average scores are can be interpreted as the corresponding geometric means.
...
[[what did we use to establish ground truth, i.e. wILI as of EWXX of 20XX/20XX season, also, specify that off-season weeks are excluded from competition]]
...
\subsection{Specific model comparisons}
...
While often these revisions are fairly minor or non-existent, at other times, these revisions can be substantial, changing the reported wILI value by over an entire percentage point, or over 50\% of the originally reported value.
Since this data is used by forecasters to generate current forecasts, these forecasts can be contaminated by the initially reported, biased data.
...
We then created a categorical variable ($X$) with a binned representation of these differences using the following six categories: (-3.5,-2.5], (-2.5,-1.5], ..., (1.5,2.5].
Using  only the forecasting results from the four most accurate individual non-ensemble models, ({\tt ReichLab-KCDE}, {\tt LANL-DBM}, {\tt Delphi-DeltaDensity1}, {\tt CU-EKF\_SIRS}), we then fit the following linear regression model
\begin{equation}
S_i = \beta + \alpha_{m(i)} + \gamma_{t(i)} + \lambda_{w(i)} + {\mathbf \theta}*X_i + \epsilon_i
\end{equation}
where the index $i$ indexes a specific set of subscripts $\{m,r,t,s,w\}$, and the $\alpha_{m(i)}$, $\gamma_{t(i)}$, and $\lambda_{w(i)}$ are model-, target-, and week- specific fixed effects. (The notation $m(i)$ refers to the model contained in the ith observation of the dataset.) The error term is assumed to follow a Gaussian distribution. The parameter of interest in the model is the vector ${\mathbf \theta}$, which represent the expected changes in average score from the reference category (defined as the bin representing changes of between +/- 0.5 from the original reported value) adjusting for model, target and week-of-season.
...
In practice, this dichotomized distinction can be fuzzy.
For example, statistical models for infectious disease counts may encode an autoregressive term for incidence, which in certain setting can be seen as representing a transmission process from one time period to another.
...
We categorized models according to whether or not they had an explicit compartmental framework. 
...
models having at least one season with greater average forecast skill than the 
{\tt \Sexpr{sanitize(max_logscore_model)}}
did.

<<results-model-season, fig.height=5, fig.cap="Average forecast score, aggregated across targets and plotted separately for each model and season. Models are sorted from lowest scores (left) to highest scores (right). Higher scores indicate better performance. Dots show average score across all targets and regions for a given season. The x marks the geometric mean of the seven seasons. The names of compartmental models are shown in bold face. The {\\tt ReichLab-KDE} model (italicized red font) can be thought of as the historical baseline model.">>=
...
<<results-model-region-tt, fig.cap="Average forecast score by model region and target-type, averaged over weeks and seasons. The white midpoint of the color scale is set to be the target-specific average of the historical baseline model, {\\tt ReichLab-KDE}.">>=
scores_by_model_targettype_region %>%
    ...
    ggplot(aes(x=Location, y=Model, fill=pct_diff_baseline_skill)) + 
...
top_model_wkahead_scores <- scores_by_model_season_targettype_region %>%
    ...
    .$avg_logscore
@
Even within given models, week-ahead forecast skill showed large region-to-region and year-to-year variation. 
The forecast skill for specific region-seasons shown by the high-accuracy {\tt \Sexpr{sanitize(max_logscore_model_wkahead)}} model varied from 
...
\Sexpr{best_region_wkahead} was the most predictable and \Sexpr{worst_region_wkahead} was the least predictable.
In \Sexpr{best_region_wkahead}, the \Sexpr{n_models} models showed an average forecast score of \Sexpr{specify_decimal(best_region_wkahead_skill, 2)} for $k$-week-ahead targets (Figure \ref{fig:map-results}). 
This means that in a typical season these models together assigned an average of \Sexpr{specify_decimal(best_region_wkahead_skill, 2)} probability to the eventually observed wILI percentages.
...
\Sexpr{best_region_wkahead} showed both the best overall week-ahead predictability of any region. This resulted from having the highest score from the baseline baseline model and having the largest improvement upon baseline predictions from the other models (Figure \ref{fig:map-results}B).  
In \Sexpr{worst_region_wkahead} the average week-ahead skill was \Sexpr{specify_decimal(worst_region_wkahead_skill, 2)}. 
While \Sexpr{worst_region_wkahead} showed the lowest baseline model score of any region, it also showed the second highest improvement upon baseline predictions (Figure \ref{fig:map-results}B).
...
wkahead_map <- hhs_regions %>% 
    merge(scores_for_map_wkahead, by="region_number") %>%
...
wkahead_scatter <- ggplot(kde_scores_wkahead, aes(x=exp(kde_logscore), y=exp(avg_logscore)-exp(kde_logscore))) + 
    ...
    ggtitle("B: vs. historical forecast") #+


## make seasonal plots
seasonal_map <- hhs_regions %>% 
    merge(scores_for_map_seasonal, by="region_number") %>%
    ggplot(aes(map_id = state)) + 
    geom_map(aes(fill = exp(avg_logscore)), map = fifty_states) +
    scale_fill_gradient(low="#f7fbff", high="#084594", limit=skill_lims, name="skill") + 
    expand_limits(x = fifty_states$long, y = fifty_states$lat) +
...
seasonal_scatter <- ggplot(kde_scores_seasonal, aes(x=exp(kde_logscore), y=exp(avg_logscore)-exp(kde_logscore))) + 
    ...
    scale_y_continuous(limits=c(0, .2), name = "score diff. w/historical model") +
    scale_x_continuous(limits=c(.1, .4),name = "score of historical model") +
    ggtitle("D: vs. historical forecast") #+


grid.arrange(wkahead_map, wkahead_scatter, seasonal_map, seasonal_scatter, ncol=2)
@
...
For the model with highest forecast skill across all four week-ahead targets ({\tt \Sexpr{sanitize(max_logscore_model_wkahead)}}), the average skill across region and season for 1 through 4 week-ahead forecasts were 
...
Only in \Sexpr{paste(regions_above_.5_best_model, collapse=" and ")} were the forecast skills from the {\tt \Sexpr{sanitize(max_logscore_model_wkahead)}} model for both the ``nowcast'' targets (1 and 2 weeks ahead) above 0.5.
...
Overall, forecast skill was lower for seasonal targets than for week-ahead targets (Figure \ref{fig:results-model-region-tt}).
...
The best single model across all seasonal targets was {\tt \Sexpr{max_logscore_model_seasonal}} with an overall forecast skill of 
\Sexpr{specify_decimal(exp(max_logscore_seasonal), 2)}.
The historical average model achieved  an overall forecast skill of 
...
Of the three seasonal targets, models showed the lowest average skill in forecasting season onset, with an overall average skill of 
...
with overall average skill of 
\Sexpr{specify_decimal(exp(max_logscore_onset), 2)}
and region-season-specific skills for onset that ranged from
...
\Sexpr{specify_decimal(exp(kde_logscore_onset), 2)} average
skill in forecasting onset.
Overall, \Sexpr{n_above_kde_onset} of \Sexpr{n_models} models (\Sexpr{round(n_above_kde_onset/n_models*100)}\%) had more forecast skill than the historical baseline model in the scoring period of interest.
...
Models showed an overall average skill of 
...
Region- and season-specific forecast skill from this model for peak week ranged from
...
skill in forecasting peak week.
Overall, \Sexpr{n_above_kde_pkwk} of \Sexpr{n_models} models (\Sexpr{round(n_above_kde_pkwk/n_models*100)}\%) had more forecast skill than the historical baseline model in the scoring period of interest.

Models showed the an overall average skill of 
...
Region- and season-specific forecast skill from this model for peak intensity ranged from
...
skill in forecasting peak intensity.
Overall, \Sexpr{n_above_kde_pk} of \Sexpr{n_models} models (\Sexpr{round(n_above_kde_pk/n_models*100)}\%) had more forecast skill than the historical baseline model in the scoring period of interest for peak intensity.
[[Check to see if 16 models better than historical were the same for all three seasonal targets... maybe make a table?]]
...
colnames(skills_to_show) <- c("target", "stat. model skill", "compartmental model skill", "difference")
...
On the whole, statistical models showed the same amount of skill as compartmental models at forecasting week-ahead targets, and slightly more skill for the seasonal targets, although the differences were small. 
...
For the week-ahead forecasts, the difference in model skill was slight, never greater than 
\Sexpr{specify_decimal(max(scores_by_modeltype$diff_model_skill[1:4])*100, 1)}
...
<<delay-analysis, fig.height=4, fig.cap="Model-estimated changes in forecast skill due to bias in initial reports of wILI \\%. The figure shows estimated coefficient values (and 95\\% confidence intervals) from a multivariable linear regression using model, week-of-year, target, and a categorized version of the bias in the first reported wILI \\% to predict forecast skill. [[The model was fit to]]  The x-axis labels show the range of bias (e.g. ``(-0.5,0.5]'' represents all observations whose first observations were within +/- 0.5 percentage points of the final reported value). Values to the left of the dashed grey line are observations whose first reported value were lower than the final. Y-axis values of less than zero (the reference category) represent decreases in expected forecast skill. The total number of observations in each category are shown above the x-axis labels.">>=
...
In the seven years examined in this study, wILI percentages were often revised after first being reported. For example, $\Sexpr{per_change_gt_20}$\% of all weekly reported wILI percentages ended up being over 20\% different than the initally reported value.
...
We found that larger biases in the initially reported data were strongly associated with a decrease in the forecast skill for the forecasts made using the incomplete data.
Specifically, among the four top-performing models we observed an expected change in forecast skill of \Sexpr{specify_decimal(fm4_coefs_fltr$estimate[fm4_coefs_fltr$bias_range == "(-3.5,-2.5]"], 2)} 
...
Importance of this work (and the foundational work of the CDC) in establishing and evaluating against a set of shared benchmarks against which other methods/models can use as comparisons.
SciSci ref on collaboration (Fortunato).
Models improve substantially on baseline forecasts, both in places where we already forecast well and not (figure 4B, D)
...
The statistical vs. mechanistic model distinction is not always a clean distinction.

Notes on mech vs. statistical comparison: mechanistic models are modeling a disease, while the data-generating mechanism for wILI captures much more than just disease transmission (e.g., clinical visitation process, symptomatic diagnosis process, reporting process, a backfill process, etc.) That is, a disease transmission model and the wILI data generating model are fundamentally different things-suggesting a limitation to purely mechanistic models. 
...
\begin{itemize}
    ...
    \item no external benchmark defined by the CDC (or others) as to what constitutes a `good' or `useful' forecast. So far, all comparisons are relative in nature. It'd be nice if the CDC said a forecast skill of, for example, 0.7 or better is useful.
\end{itemize}
