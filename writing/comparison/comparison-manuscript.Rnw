...
While multi-model comparisons exist in the literature for single-season performance \cite{Biggerstaff2016,Biggerstaff2018}, here we compare the same models over seven influenza seasons. 
...
Influenza incidence typically exhibits a strong annual periodicity in the US (and globally throughout the Northern Hemisphere), often circulating widely during colder months (i.e., November through April). 
...
This effort has galvanized a community of scientists interested in forecasting, creating an organic testbed for improving both technical understanding of how different forecast models perform and also how to integrate these models into decision-making.
...
Second, by using alternate modeling approaches, how much more accuracy can be achieved beyond the historical baseline? 
...
\begin{itemize}
    ...
    \item Forecast accuracy is significantly degraded in some regions due to reporting biases in initially reported real-time data (Figure \ref{fig:delay-analysis}).
\end{itemize}
...
Given that several of the models rely on similar modeling frameworks, adding a broader sample of modeling approaches would be a valuable contribution.
...
We see particular promise in models that leverage different data sources, such as pathogen-specific and highly localized incidence or public health surveillance data.
...
To advance the enterprise of infectious disease forecasting broadly, a complete enumeration and understanding of the challenges facing the field is critical.
...
Often, researchers either lack access to or do not know how best to make use of novel data streams (e.g., Internet data, electronic medical health record data) to the fullest extent possible. 
