...
\Sexpr{specify_decimal(min(scores_by_model_targettype_region$avg_logscore[which(scores_by_model_targettype_region$Model==max_logscore_model_wkahead & scores_by_model_targettype_region$target_type == "k-week-ahead" )]), 2)} 
and 
\Sexpr{specify_decimal(max(scores_by_model_targettype_region$avg_logscore[which(scores_by_model_targettype_region$Model==max_logscore_model_wkahead & scores_by_model_targettype_region$target_type == "k-week-ahead" )]), 2)}.
As a comparison, the historical baseline model achieved between
\Sexpr{specify_decimal(min(scores_by_model_targettype_region$avg_logscore[which(scores_by_model_targettype_region$Model=="ReichLab-KDE" & scores_by_model_targettype_region$target_type == "k-week-ahead" )]), 2)} and
\Sexpr{specify_decimal(max(scores_by_model_targettype_region$avg_logscore[which(scores_by_model_targettype_region$Model=="ReichLab-KDE" & scores_by_model_targettype_region$target_type == "k-week-ahead" )]), 2)}
...
The forecask skill for specific region-seasons shown by the high-accuracy {\tt \Sexpr{sanitize(max_logscore_model_wkahead)}} model varied from 
...
On the flip side, in \Sexpr{worst_region_wkahead} the average skill was \Sexpr{specify_decimal(worst_region_wkahead_skill, 2)}. 
This was just marginally better than the forecast skill for the historical average model in \Sexpr{worst_region_wkahead}, which was \Sexpr{specify_decimal(kde_kwkahead_worst_score,2)}.
...
<<map-results, fig.height=4.5, fig.cap="(A \\& C) Forecast skill for week-ahead and seasonal targets. Color shading indicates average forecast skill for all models. (B \\& D) Comparisons of historical model skill (x-axis) with the average skill of all models (y-axis). Positive y-values indicate an improvement over the historical baseline. The digits in the plot refer to the corresponding HHS Region number.">>=
...
For example, $\Sexpr{per_change_gt_20}$\% of all weekly reported wILI percentages ended up being over 20\% different than the originally reported value.
...
Specifically, among top-performing models we see an expected change in forecast skill of $\Sexpr{round(fm4_coefs_fltr$estimate[fm4_coefs_fltr$bias_range == "(-3.5,-2.5]"], 2)}$ when the first observed wILI measurement is between 2.5 and 3.5 percentage points lower than the final observed value, adjusting for model, week-of-year, and target (Figure \ref{fig:delay-model-coefs}).
