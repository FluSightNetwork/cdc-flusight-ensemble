...
The model with the highest average week-ahead forecast skill across all regions and seasons was {\tt \Sexpr{sanitize(max_logscore_model_wkahead)}}.
...
<<results-model-region-tt, fig.cap="Average forecast skill by model region and target-type, averaged over weeks and seasons. The white midpoint of the color scale is set to be the overall average of the historical baseline model, {\\tt ReichLab-KDE}.">>=
midpt <- mean(exp(filter(scores_by_model_targettype_region, Model=="ReichLab-KDE")$avg_logscore))
ggplot(scores_by_model_targettype_region, 
    aes(x=Location, fill=exp(avg_logscore), y=Model)) + 
...
In \Sexpr{best_region_wkahead}, the \Sexpr{n_models} models showed an average forecast skill of \Sexpr{specify_decimal(best_region_wkahead_skill, 2)} for $k$-week-ahead targets (Figure \ref{fig:map-results}). 
...
On the flip side, in \Sexpr{worst_region_wkahead} the average week-ahead skill was \Sexpr{specify_decimal(worst_region_wkahead_skill, 2)}. 
...
wkahead_scatter <- ggplot(kde_scores_wkahead, aes(x=exp(kde_logscore), y=exp(avg_logscore)-exp(kde_logscore))) + 
    ...
    #geom_hline(yintercept=0, linetype=2) + 
    scale_y_continuous(limits=c(0, .2), name = "skill diff. w/historical model") +
    scale_x_continuous(limits=c(.1, .4), name = "historical model skill") +
...
seasonal_map <- hhs_regions %>% 
    ...
    scale_y_continuous(breaks = NULL) +
    labs(x = NULL, y = NULL) +
    ggtitle("C: seasonal forecast skill")
...
Following CDC evaluation rules, peak week and peak intensity forecasts were scored for all weeks in a specific region-season up until the wILI measure drops below the regional baseline level for the final time. 
...
percentange points of probability. 
...
In the seven years examined in this study, wILI percentages were often revised after first being reported. For example, $\Sexpr{per_change_gt_20}$\% of all weekly reported wILI percentages ended up being over 20\% different than the originally reported value.
