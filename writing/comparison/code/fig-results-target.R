...
midpt <- mean(filter(scores_by_target, Model=="ReichLab-KDE")$skill)
p <- ggplot(scores_by_target, 
  aes(x=Target, fill=skill, y=Model)) + 
  geom_tile() + ylab(NULL) + xlab(NULL) +
  geom_text(aes(label=round(skill, 2))) +
  scale_fill_gradient2(midpoint = midpt) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ggtitle("Average forecast skill by target")
